...
def generate_launch_description():
    ...
    srdf_file = load_file('omron1s_moveit2_config', 'config/omron1s.srdf')
    robot_description_semantic = {"robot_description_semantic": srdf_file}

    kinematics_yaml = load_yaml(
        "omron1s_moveit2_config", "config/kinematics.yaml"
    )

    # MoveGroupInterface demo executable
    move_group_demo = Node(
        name="test_trajectory_multi",
        package="omron1s_scripts",
        executable="test_trajectory_multi",
        output="screen",
        parameters=[
            {'use_sim_time': True},
            robot_description,
